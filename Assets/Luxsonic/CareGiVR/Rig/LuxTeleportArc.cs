...
public class LuxTeleportArc : MonoBehaviour
{
        ...
        private float FindProjectileCollision(out RaycastHit hitInfo)
        {
            ...
            for (int i = 0; i < segmentCount; ++i)
            {
                ...
                if (Physics.Linecast(segmentStartPos, segmentEndPos, out hitInfo, traceLayerMask.value))
                {
                    if (true /* hitInfo.collider.GetComponent<IgnoreTeleportTrace>() == null */)
                    {
                        LuxUtil.DrawCross(hitInfo.point, Color.red, 0.5f);
