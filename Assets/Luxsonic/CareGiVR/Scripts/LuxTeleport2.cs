...
namespace Caregivr
{
public class LuxTeleport2 : MonoBehaviour
{
		...
		private LuxHand pointerHand = null;
		private LuxRig player = null;
		private LuxTeleportArc teleportArc = null;

		private bool visible = false;

		private LuxTeleportMarkerBase[] teleportMarkers;
		private LuxTeleportMarkerBase pointedAtTeleportMarker;
		private LuxTeleportMarkerBase teleportingToMarker;
		...
		// LUX NEW
		private bool lastTeleportButtonLeft;
		private bool lastTeleportButtonRight;
		// END LUX NEW
...
/*
		...
		public static SteamVR_Events.Event< Valve.VR.InteractionSystem.TeleportMarkerBase > Player = new SteamVR_Events.Event< Valve.VR.InteractionSystem.TeleportMarkerBase >();
		public static SteamVR_Events.Action< Valve.VR.InteractionSystem.TeleportMarkerBase > PlayerAction( UnityAction< Valve.VR.InteractionSystem.TeleportMarkerBase > action ) { return new SteamVR_Events.Action< Valve.VR.InteractionSystem.TeleportMarkerBase >( Player, action ); }

		public static SteamVR_Events.Event< Valve.VR.InteractionSystem.TeleportMarkerBase > PlayerPre = new SteamVR_Events.Event< Valve.VR.InteractionSystem.TeleportMarkerBase >();
		public static SteamVR_Events.Action< Valve.VR.InteractionSystem.TeleportMarkerBase > PlayerPreAction( UnityAction< Valve.VR.InteractionSystem.TeleportMarkerBase > action ) { return new SteamVR_Events.Action< Valve.VR.InteractionSystem.TeleportMarkerBase >( PlayerPre, action ); }
*/
		//-------------------------------------------------
		private static LuxTeleport2 _instance;
		public static LuxTeleport2 instance
		{
			get
			{
				if ( _instance == null )
				{
					_instance = GameObject.FindObjectOfType<LuxTeleport2>();
				}
		...
		void Awake()
        {
			...
			teleportArc = GetComponent<LuxTeleportArc>();
		...
		void Start()
        {
            teleportMarkers = GameObject.FindObjectsOfType<LuxTeleportMarkerBase>();

			HidePointer();

			player = FindObjectOfType<LuxRig>();

			if ( player == null )
			{
				Debug.LogError("[LuxTeleport2] No Player instance found.");
		...
		{
			foreach ( LuxTeleportMarkerBase teleportMarker in teleportMarkers )
			{
				/*
				Valve.VR.InteractionSystem.TeleportPoint teleportPoint = teleportMarker as Valve.VR.InteractionSystem.TeleportPoint;
		...
		void Update()
		{
			...
			LuxHand oldPointerHand = pointerHand;
			LuxHand newPointerHand = null;

			foreach ( LuxHand hand in player.hands )
			{
				if ( visible )
				{
			...
			lastTeleportButtonLeft = IsTeleportButtonDown(player.luxLeftHand);
			lastTeleportButtonRight = IsTeleportButtonDown(player.luxRightHand);
		}
		...
		private void UpdatePointer()
		{
			...
			LuxTeleportMarkerBase hitTeleportMarker = null;
			...
			{
				hitSomething = true;
				hitTeleportMarker = hitInfo.collider.GetComponentInParent<LuxTeleportMarkerBase>();
			}
			...
			if ( hitTeleportMarker != null ) //Hit a teleport marker
			{
				...
				if (pointedAtTeleportMarker is LuxsonicTeleportPoint)
				{
					pointedAtPosition = pointedAtTeleportMarker.transform.position;
				}

				if ( showPlayAreaMarker )
				{
					//Show the play area marker if this is a teleport area
					LuxTeleportArea teleportArea = pointedAtTeleportMarker as LuxTeleportArea;
			...
			else //Hit neither
			{
				...
				float invalidReticleCurrentScale = LuxUtil.RemapNumberClamped( distanceFromPlayer, invalidReticleMinScaleDistance, invalidReticleMaxScaleDistance, invalidReticleMinScale, invalidReticleMaxScale );
		...
		*/
		...
		//-------------------------------------------------
		private void ShowPointer( LuxHand newPointerHand, LuxHand oldPointerHand )
		{
			if ( !visible )
			{
				pointedAtTeleportMarker = null;
...
*/
		...
		private void InitiateTeleportFade()
		{
			...
			/*
			Valve.VR.InteractionSystem.TeleportPoint teleportPoint = teleportingToMarker as Valve.VR.InteractionSystem.TeleportPoint;
			if ( teleportPoint != null && teleportPoint.teleportType == Valve.VR.InteractionSystem.TeleportPoint.TeleportPointType.SwitchToNewScene )
			{
				currentFadeTime *= 3.0f;
				Valve.VR.InteractionSystem.Teleport.ChangeScene.Send( currentFadeTime );
			}
			*/
		...
		private void TeleportPlayer()
		{
			...
			//Valve.VR.InteractionSystem.Teleport.PlayerPre.Send( pointedAtTeleportMarker );
			...
			if ( teleportPoint != null )
			{
				...
				if ( teleportPoint.teleportType == Valve.VR.InteractionSystem.TeleportPoint.TeleportPointType.SwitchToNewScene )
				{
			...
			*/
			...
			LuxTeleportArea teleportArea = teleportingToMarker as LuxTeleportArea;
			if ( teleportArea != null )
			{
				if ( floorFixupMaximumTraceDistance > 0.0f )
				{
			...
			if ( teleportingToMarker.ShouldMovePlayer() )
			{
				...
				player.fadeSphere.Fade(delegate()
				{
					player.trackingOriginTransform.position = teleportPosition;
					teleportingToMarker.OnTeleport();
				});
				
				/*
                if (player.luxLeftHand.currentAttachedObjectInfo.HasValue)
                    player.luxLeftHand.ResetAttachedTransform(player.luxLeftHand.currentAttachedObjectInfo.Value);
                if (player.luxRightHand.currentAttachedObjectInfo.HasValue)
                    player.luxRightHand.ResetAttachedTransform(player.luxRightHand.currentAttachedObjectInfo.Value);
				*/
            }
			else
			{
			...
			//Valve.VR.InteractionSystem.Teleport.Player.Send( pointedAtTeleportMarker );
		}


		//-------------------------------------------------
		private void HighlightSelected( LuxTeleportMarkerBase hitTeleportMarker )
		{
		...
		*/
		...
		//-------------------------------------------------
		public bool IsEligibleForTeleport( LuxHand hand )
		{
			if ( hand == null )
			{
				return false;
		...
		private bool LastPressedForHand(LuxHand hand)
		{
			if (hand.handDirection == LuxHand.LuxHandDirection.Left)
			{
				return lastTeleportButtonLeft;
			}
			return lastTeleportButtonRight;
		}

		//-------------------------------------------------
		private bool WasTeleportButtonReleased( LuxHand hand )
		{
			return !IsTeleportButtonDown(hand) && LastPressedForHand(hand);
			// LUX TODO
		...
		//-------------------------------------------------
		private bool IsTeleportButtonDown( LuxHand hand )
		{
			// LUX TODO
			return hand.controller != null && hand.controller.Teleport();
		...
		//-------------------------------------------------
		private bool WasTeleportButtonPressed( LuxHand hand )
		{
			return IsTeleportButtonDown(hand) && !LastPressedForHand(hand);
			// LUX TODO
			/*
		...
		//-------------------------------------------------
		private Transform GetPointerStartTransform( LuxHand hand )
		{
			/*
			if ( hand.noSteamVRFallbackCamera != null )
			{
			...
			*/
			return hand.teleportArcTransform;
		}
}
